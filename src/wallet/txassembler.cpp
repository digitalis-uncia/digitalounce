    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional DOZ outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes DOZ and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::DOZ_TO_DOZ) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        if (SelectCoins(new_tx, nTargetValue, params_pegin)) {
            return std::any_of(new_tx.selected_coins.cbegin(), new_tx.selected_coins.cend(), is_doz);
        }
    } else {
        // First try to construct a DOZ-to-DOZ transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::DOZ_ONLY;
        ...
        // If DOZ-to-DOZ fails, create a peg-out transaction
