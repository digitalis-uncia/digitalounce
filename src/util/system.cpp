...
const char * const BITCOIN_CONF_FILENAME = "digitalounce.conf";
...
#else
    const char* pszModule = "digitalounce";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Digitalounce";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Digitalounce";
#else
    // Unix-like
    return pathRet / ".digitalounce";
#endif
#endif
}
