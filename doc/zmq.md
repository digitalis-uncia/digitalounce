...
The Digitalounce Core daemon can be configured to act as a trusted "border
router", implementing the digitalounce wire protocol and relay, making
...
The ZeroMQ feature in Digitalounce Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building digitalounced:
...
For instance:
    ...
    $ digitalounced -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/digitalounced.tx.raw \
...
These options can also be provided in digitalounce.conf.
...
From the perspective of digitalounced, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into digitalounced directly. Furthermore, no information is
...
using. Digitalounced appends an up-counting sequence number to each
