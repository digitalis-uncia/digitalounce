Digitalounce Core version *0.15.1* is now available from:

  <https://download.digitalounce.org/digitalounce-0.15.1/>
...
A number of changes to the way Digitalounce Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 grams per kB (0.00001 DOZ). This is to prepare for dropping the minimum transaction fee to 0.1 grams per kB (0.0001 DOZ) in the next release.
