...
Anyone can contribute to Digitalounce Core. Please scroll down to ‘How to contribute to Digitalounce Core’ for details.

By downloading or upgrading Digitalounce Core, you’re helping to secure the network, so thank you and enjoy the upgrade!
...
From Digitalounce Core 0.21.1 onwards, macOS versions earlier than 10.12 are no longer supported. Additionally, Digitalounce Core doesn’t yet change appearance when macOS "dark mode" is activated.
...
- This release adds support for Tor version 3 hidden services, and rumoring them
  ...
  [BIP155](https://github.com/Digitalounce/bips/blob/master/bip-0155.mediawiki).
  Version 2 hidden services are still fully supported by Digitalounce Core, but the
...
- The Tor onion service that is automatically created by setting the
  ...
  `-datadir`) and can be removed if not needed. Digitalounce Core will no longer
...
- This release adds support for serving
  [BIP157](https://github.com/Digitalounce/bips/blob/master/bip-0157.mediawiki) compact
  filters to peers on the network when enabled using
  `-blockfilterindex=1 -peercfilters=1`. (#16442)

- This release implements
  [BIP339](https://github.com/Digitalounce/bips/blob/master/bip-0339.mediawiki)
...
- This release implements the proposed Taproot consensus rules
  ([BIP341](https://github.com/Digitalounce/bips/blob/master/bip-0341.mediawiki) and
  [BIP342](https://github.com/Digitalounce/bips/blob/master/bip-0342.mediawiki)),
...
- A new `Digitalounce-cli -netinfo` command provides a network peer connections
...
- A new `Digitalounce-cli -generate` command, equivalent to RPC `generatenewaddress`
  followed by `generatetoaddress`, can generate blocks for command line testing
  purposes. This is a client-side version of the former `generate` RPC. See the
  help for details. (#19133)

- The `Digitalounce-cli -getinfo` command now displays the wallet name and balance for
  each of the loaded wallets when more than one is loaded (e.g. in multiwallet
  mode) and a wallet is not specified with `-rpcwallet`. (#18594)

- The `connections` field of `Digitalounce-cli -getinfo` is now expanded to return a JSON
...
- The `startupnotify` option is used to specify a command to
  execute when Digitalounce Core has finished with its startup
  sequence. (#15367)
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `Digitalounce-wallet` tool which performs the salvage
...
Digitalounce Core will no longer automatically create new wallets on startup. It will
load existing wallets specified by `-wallet` options on the command line or in
`Digitalounce.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
Digitalounce Core will now just log warnings instead of creating new wallets with
new keys and addresses like previous releases did.

New wallets can be created through the GUI (which has a more prominent create
wallet option), through the `Digitalounce-cli createwallet` or `Digitalounce-wallet
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  startup, so they don't need to be manually reloaded next time Digitalounce Core is
  started. The list of wallets to load on startup is stored in
  `\<datadir\>/settings.json` and augments any command line or `Digitalounce.conf`
